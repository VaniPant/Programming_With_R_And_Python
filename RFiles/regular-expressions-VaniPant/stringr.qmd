...
length(str_subset(agro_data$annothits, "Drug and Medication Information"))
```
...
```{r agro_with_stereo}
# Write your solution here
sum(str_detect(agro_data$isosmiles, "[@\\/]"))
```
...
```{r canonical_smiles}
# Write your solution here
sum(str_detect(agro_data$canonicalsmiles, "[@\\/]"))
```
...
```{r agrochem_from_lux}
# Write your solution here
agro_data[str_detect(agro_data$sidsrcname, "Luxembourg"),]
```
...
```{r agrochem_2023}
# Write your solution here
sum(str_detect(agro_data$cidcdate, "^2023"), na.rm = TRUE)
```
